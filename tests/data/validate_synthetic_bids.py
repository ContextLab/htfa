...
def validate_bids_structure():
    ...
    if sorted(participants) == sorted(expected_participants):
        print(f"✓ Found all {len(participants)} participants: {', '.join(sorted(participants))}")
    else:
        print(f"✗ Participant mismatch. Expected: {expected_participants}, Found: {participants}")
        return False
            ...
            if len(json_files) == 1:
                with open(json_files[0].path) as f:
                    ...
                    if metadata.get('RepetitionTime') == 2.0:
                        ...
                        print(f"    ✗ Incorrect TR in JSON: {metadata.get('RepetitionTime')}")
            else:
                print(f"    ✗ Missing JSON sidecar")
                return False
        else:
            print(f"  ✗ sub-{sub}: Found {len(func_files)} functional files (expected 1)")
            return False
    ...
    if ground_truth_path.exists():
        ...
        for sub in participants:
            ...
            if sub_key in ground_truth["participants"]:
                ...
                if all(k in participant_data for k in ["factors", "weights", "spatial_coords", "affine"]):
                    factors = np.array(participant_data["factors"])
                    weights = np.array(participant_data["weights"])
                    print(f"  ✓ sub-{sub}: Factors shape {factors.shape}, Weights shape {weights.shape}")
                else:
    ...
    if desc_path.exists():
        with open(desc_path) as f:
            ...
            if desc.get("BIDSVersion") and desc.get("Name"):
                print(f"  ✓ dataset_description.json present (BIDS v{desc['BIDSVersion']})")
            else:
    ...
    if participants_path.exists():
        ...
        if len(participants_df) == 10:
            ...
            print(f"  ✗ participants.tsv has wrong number of participants: {len(participants_df)}")
            return False
    else:
