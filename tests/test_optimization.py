...
from htfa.htfa.optimization import (
    AlternatingLeastSquares, AdamOptimizer, MiniBatchOptimizer,
    SparseOptimizer, create_optimizer, OptimizationScheduler
)
...
class TestOptimizers:
    ...
    def test_optimization_convergence(self):
        ...
        result_tight = optimizer.optimize(
            self.X, self.initial_factors, self.initial_loadings, 
            max_iter=100, tol=1e-1
        )
    ...
    def test_loss_decreasing(self):
        ...
        if len(result.loss_history) > 1:
            assert result.loss_history[-1] <= result.loss_history[0] * 1.1  # Allow some tolerance
...
class TestOptimizationWithRealData:
    ...
    def setup_method(self):
        ...
        # Add noise
        self.X = np.dot(true_factors, true_loadings) + 0.1 * np.random.randn(n_samples, n_features)
    ...
    def test_optimization_quality(self):
        ...
        initial_factors = np.random.randn(self.X.shape[0], self.n_factors).astype(np.float32)
        initial_loadings = np.random.randn(self.n_factors, self.X.shape[1]).astype(np.float32)
