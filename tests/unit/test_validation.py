...
class TestValidateArrays:
    ...
    def test_valid_4d_array(self):
        ...
        data = np.random.randn(3, 2, 100, 50)  # 3 subjects, 2 runs, 100 voxels, 50 timepoints
    ...
    def test_coords_count_mismatch(self):
        ...
        data_list = [np.random.randn(100, 50), np.random.randn(100, 60)]
        coords_list = [np.random.randn(100, 3)]  # Only one coord array for two data arrays
...
class TestCheckDataQuality:
    ...
    def test_low_variance_warning(self):
        ...
        warning_found = any('low signal variance' in w.lower() for w in report['warnings'])
        assert warning_found
        
        recommendation_found = any('preprocessing' in r.lower() for r in report['recommendations'])
    ...
    def test_large_range_warning(self):
        ...
        recommendation_found = any('normalization' in r.lower() for r in report['recommendations'])
        assert recommendation_found
    ...
    def test_large_mean_warning(self):
        ...
        recommendation_found = any('mean-centering' in r.lower() for r in report['recommendations'])
        assert recommendation_found
