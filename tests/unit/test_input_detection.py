...
from htfa.core import TFA, HTFA
...
class TestInputDetection:
    ...
    def test_missing_coords_for_array_input_raises_error(self):
        ...
        with pytest.raises(TypeError, match="coords parameter is required for array input"):
            fit(self.single_subject_data)
        
        # Multi-subject arrays without coords
        with pytest.raises(TypeError, match="coords parameter is required for multi-subject array input"):
...
class TestParameterInference:
    ...
    @pytest.mark.parametrize("n_voxels,n_timepoints,expected_factors", [
        ...
        (10000, 1000, 50), # min(sqrt(10000), 1000//10, 50) = min(100, 100, 50) = 50
...
class TestBIDSDatasetFitting:
    ...
    def test_bids_directory_not_implemented_error(self):
        ...
        with pytest.raises(NotImplementedError, match="BIDS dataset parsing will be implemented"):
    ...
    def test_nifti_loading_not_implemented_error(self):
        ...
        with pytest.raises(NotImplementedError, match="NIfTI file loading will be implemented"):
...
class TestArraysFitting:
    ...
    def test_coordinate_mismatch_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Coordinates shape 50 doesn't match data voxels 100"):
