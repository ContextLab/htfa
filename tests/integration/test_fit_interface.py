...
class TestFitInterfaceIntegration:
    ...
    def _generate_synthetic_fmri_data(
        ...
        for f in range(n_factors):
            ...
            for i in range(1, n_voxels - 1):
                smoothed[i] = 0.5 * factors[i, f] + 0.25 * (factors[i-1, f] + factors[i+1, f])
            factors[:, f] = smoothed
        ...
        for f in range(n_factors):
            for t in range(1, n_timepoints - 1):
                weights[f, t] = 0.6 * weights[f, t] + 0.2 * (weights[f, t-1] + weights[f, t+1])
    ...
    def test_parameter_inference_accuracy(self):
        ...
        for n_vox, n_time in test_cases:
            ...
            data = self._generate_synthetic_fmri_data(n_vox, n_time, 8)  # 8 true factors
    ...
    def test_data_quality_robustness(self):
        ...
        noisy_data = self.single_subject_data + np.random.randn(*self.single_subject_data.shape) * 0.5
        ...
        centered_data = self.single_subject_data - np.mean(self.single_subject_data, axis=1, keepdims=True)
...
class TestErrorHandlingIntegration:
    ...
    def test_invalid_coordinate_dimensions(self):
        ...
        with pytest.raises(ValueError, match="Coordinates shape.*doesn't match.*data voxels"):
