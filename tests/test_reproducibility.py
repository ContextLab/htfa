...
class TestTFAReproducibility:
    ...
    def test_tfa_reproducibility_with_fixed_seed(self, sample_data):
        ...
        for i in range(1, len(results)):
            np.testing.assert_array_almost_equal(
                results[0]['factors'], results[i]['factors'],
                decimal=10, err_msg=f"Factors differ between run 0 and run {i}"
            )
            np.testing.assert_array_almost_equal(
                results[0]['weights'], results[i]['weights'],
                decimal=10, err_msg=f"Weights differ between run 0 and run {i}"
            )
            np.testing.assert_array_almost_equal(
                results[0]['centers'], results[i]['centers'],
                decimal=10, err_msg=f"Centers differ between run 0 and run {i}"
            )
            np.testing.assert_array_almost_equal(
                results[0]['widths'], results[i]['widths'],
                decimal=10, err_msg=f"Widths differ between run 0 and run {i}"
            )
    ...
    def test_tfa_none_random_state_is_non_deterministic(self, sample_data):
        ...
        assert not all_same, "Results with random_state=None should be non-deterministic"
    ...
    def test_tfa_subsampling_reproducibility(self, sample_data):
        ...
        tfa1 = TFA(K=3, random_state=42, max_num_voxel=50, max_num_tr=30, 
                  max_iter=5, verbose=False)
        tfa1.fit(data, coords)
        
        tfa2 = TFA(K=3, random_state=42, max_num_voxel=50, max_num_tr=30,
                  max_iter=5, verbose=False)
        tfa2.fit(data, coords)
    ...
    def test_tfa_kmeans_initialization_reproducibility(self, sample_data):
        ...
        for _ in range(3):
            tfa = TFA(K=4, random_state=42, max_iter=1, verbose=False)  # Only 1 iteration
            tfa.fit(data, coords)
...
class TestHTFAReproducibility:
    ...
    def test_htfa_reproducibility_with_fixed_seed(self, multi_subject_data):
        ...
        for _ in range(2):  # Fewer runs due to computational cost
            htfa = HTFA(K=3, random_state=42, max_global_iter=3, max_local_iter=5, verbose=False)
            htfa.fit(X, coords)
            results.append({
                'global_template': htfa.get_global_template(),
                'subject_factors': [htfa.get_subject_factors(i) for i in range(len(X))],
                'subject_weights': [htfa.get_subject_weights(i) for i in range(len(X))]
            })
        
        # Verify results are identical
        # Global template comparison
        if results[0]['global_template'] is not None and results[1]['global_template'] is not None:
            for key in ['centers', 'widths']:
                np.testing.assert_array_almost_equal(
                    results[0]['global_template'][key],
                    results[1]['global_template'][key],
                    decimal=8, err_msg=f"Global template {key} differs between runs"
                )
        
        # Subject-specific results comparison
        for subject_idx in range(len(X)):
            if (results[0]['subject_factors'][subject_idx] is not None and
                results[1]['subject_factors'][subject_idx] is not None):
                np.testing.assert_array_almost_equal(
                    results[0]['subject_factors'][subject_idx],
                    results[1]['subject_factors'][subject_idx],
                    decimal=6, err_msg=f"Subject {subject_idx} factors differ"
                )
            
            if (results[0]['subject_weights'][subject_idx] is not None and
                results[1]['subject_weights'][subject_idx] is not None):
                np.testing.assert_array_almost_equal(
                    results[0]['subject_weights'][subject_idx],
                    results[1]['subject_weights'][subject_idx],
                    decimal=6, err_msg=f"Subject {subject_idx} weights differ"
                )
    ...
    def test_htfa_different_seeds_produce_different_results(self, multi_subject_data):
        ...
        htfa1 = HTFA(K=3, random_state=42, max_global_iter=3, max_local_iter=5, verbose=False)
        htfa1.fit(X, coords)
        
        htfa2 = HTFA(K=3, random_state=123, max_global_iter=3, max_local_iter=5, verbose=False)
        htfa2.fit(X, coords)
        ...
        assert differences_found, "Different random_state should produce different results"
    ...
    def test_htfa_none_random_state_is_non_deterministic(self, multi_subject_data):
        ...
        for _ in range(2):  # Limited runs due to computational cost
            htfa = HTFA(K=3, random_state=None, max_global_iter=2, max_local_iter=3, verbose=False)
            htfa.fit(X, coords)
        ...
        if len(results[0]) > 0 and len(results[1]) > 0:
            ...
            for i in range(min(len(results[0]), len(results[1]))):
                try:
                    np.testing.assert_array_almost_equal(results[0][i], results[1][i], decimal=5)
                except AssertionError:
                    all_same = False
                    break
            ...
            if all_same:
                print("Warning: Non-deterministic results were identical (low probability event)")
    ...
    def test_htfa_individual_subject_models_use_random_state(self, multi_subject_data):
        ...
        htfa = HTFA(K=3, random_state=42, max_global_iter=1, max_local_iter=3, verbose=False)
        htfa.fit(X, coords)
        
        # Extract the individual TFA models and check they were initialized consistently
        assert hasattr(htfa, 'subject_models_'), "HTFA should have subject_models_ after fitting"
        assert len(htfa.subject_models_) == len(X), "Should have one TFA model per subject"
...
class TestEdgeCases:
    ...
    def test_single_subject_htfa(self):
        ...
        htfa1 = HTFA(K=3, random_state=42, max_global_iter=2, max_local_iter=3, verbose=False)
        htfa1.fit(X, coords_list)
        
        htfa2 = HTFA(K=3, random_state=42, max_global_iter=2, max_local_iter=3, verbose=False)
...
if __name__ == "__main__":
    ...
    htfa1 = HTFA(K=3, random_state=42, max_global_iter=2, max_local_iter=3, verbose=True)
    htfa1.fit(X, coords_list)
    
    htfa2 = HTFA(K=3, random_state=42, max_global_iter=2, max_local_iter=3, verbose=True)
    htfa2.fit(X, coords_list)
    
    print("HTFA subject 0 factors match:", 
