...
class HTFABenchmark:
        ...
        for backend in backends:
            try:
                result = self.benchmark_htfa(
                    ...
                    n_factors,
                    backend=backend,
                    name=f"Backend_Comparison_{backend}",
                    **kwargs,
                )
                results.append(result)
                print(
                    f"✓ {backend}: {result.execution_time:.3f}s, "
        ...
        **kwargs
        ...
        for scale in scale_factors:
            ...
            result = self.benchmark_htfa(
                ...
                n_factors,
                backend=backend,
                name=f"Scaling_{backend}_{scale}x",
                **kwargs,
            )
            results.append(result)

            print(
                f"Scale {scale}x: {shape} -> {result.execution_time:.3f}s, "
    ...
    def profile_memory_usage(
        ...
        for shape in data_shapes:
            ...
            print(
                f"Shape {shape}: Peak memory {result.peak_memory:.1f}MB, "
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"{'Name':<30} {'Time(s)':<10} {'Memory(MB)':<12} {'Error':<12} {'Shape':<15}"
        )
        print("-" * 80)

        for result in self.results:
            print(
                f"{result.name:<30} {result.execution_time:<10.3f} "
        ...
        print(
            f"Lowest Memory: {lowest_memory.name} ({lowest_memory.memory_usage:.1f}MB)"
        )
        print(
            f"Lowest Error: {lowest_error.name} ({lowest_error.reconstruction_error:.6f})"
        )
        print("=" * 80)
