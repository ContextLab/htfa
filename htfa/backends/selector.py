...
class BackendSelector:
        ...
        if self.libraries["jax"]["available"]:
            if self.libraries["jax"]["tpu_available"]:
                ...
                if self.hardware["memory"]["total_mb"] >= 8192:  # 8GB+
                    scores["jax"] = (
                        self.BACKEND_SCORES["jax_cpu"],
                        "JAX with JIT compilation",
                    )
                else:
                    scores["jax"] = (
                        self.BACKEND_SCORES["jax_cpu"] - 10,
    ...
    def select_backend(self, backend: Optional[str] = None) -> str:
        ...
        if backend == "jax" and not self.hardware["has_gpu"]:
            logger.info(
                "JAX selected for CPU - will use JIT compilation for performance"
            )
        elif backend == "pytorch" and not self.hardware["has_gpu"]:
            logger.info(
                "PyTorch selected for CPU - consider JAX for better CPU performance"
            )
        elif backend == "numpy":
            if (
                not self.libraries["jax"]["available"]
                and not self.libraries["pytorch"]["available"]
            ):
                logger.info(
                    "NumPy selected - install JAX or PyTorch for better performance"
                )
            else:
                logger.warning(
                    "NumPy selected despite other backends being available - check configuration"
                )
    ...
    def estimate_memory_requirement(
        self, n_voxels: int, n_times: int, k_components: int
    ) -> int:
    ...
    def validate_selection(
        self, backend: str, n_voxels: int, n_times: int, k_components: int
    ) -> bool:
        ...
        if backend in ["jax", "pytorch"] and self.hardware["has_gpu"]:
            ...
            if gpu_memory and required_mb > gpu_memory * 0.8:  # Leave 20% buffer
                logger.warning(
                    f"Problem size ({required_mb}MB) may exceed GPU memory ({gpu_memory}MB)"
                )
                return False
        ...
        if required_mb > available_mb * 0.8:  # Leave 20% buffer
            logger.warning(
                f"Problem size ({required_mb}MB) may exceed available memory ({available_mb}MB)"
            )
            return False
...
def validate_backend_selection(
    backend: str, n_voxels: int, n_times: int, k_components: int
) -> bool:
...
def clear_backend_cache():
    ...
    from ..utils import clear_detection_cache, clear_hardware_cache
