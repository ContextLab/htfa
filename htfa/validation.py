...
def validate_arrays(
    ...
    if coords is not None:
        if isinstance(coords, list):
            ...
            try:
                coords_validated = check_array(coords, dtype=np.float64, ensure_2d=True)
            except ValueError as e:
                raise ValidationError(
        ...
        if isinstance(data, list):
            expected_n_voxels = data_validated.shape[
                1
            ]  # After stacking: (n_subjects, n_voxels, n_timepoints)
        else:
...
def check_data_quality(
    ...
    """
    report: Dict[str, Any] = {"warnings": [], "metrics": {}, "recommendations": []}
