    ...
    def optimize(
        ...
        for iteration in range(max_iter):
            ...
            loss += self.regularization * (
                np.linalg.norm(factors) ** 2 + np.linalg.norm(loadings) ** 2
            )
...
class OptimizationScheduler:
    ...
    def step_decay(
        self, epoch: int, drop_rate: float = 0.5, epochs_drop: int = 10
    ) -> float:
        """Step decay schedule."""
    ...
    def cosine_decay(self, epoch: int, total_epochs: int) -> float:
        ...
        self.current_lr = (
            self.initial_lr * 0.5 * (1 + np.cos(np.pi * epoch / total_epochs))
        )
        return self.current_lr
