...
class NumPyBackend(HTFABackend):
    ...
    def norm(self, a: np.ndarray, axis: Optional[int] = None, keepdims: bool = False) -> np.ndarray:
        return np.linalg.norm(a, axis=axis, keepdims=keepdims)
    
    def mean(self, a: np.ndarray, axis: Optional[int] = None, keepdims: bool = False) -> np.ndarray:
...
class HTFA:
    ...
    def _update_factors(self, X: Any, loadings: Any) -> Any:
        ...
        # Add regularization
        reg_term = self.regularization * self.backend.array(
            np.eye(LLT.shape[0])
        )
        ...
        LLT_inv = self.backend.matmul(
            self.backend.matmul(self.backend.transpose(Vt), 
                               self.backend.array(np.diag(s_inv))),
