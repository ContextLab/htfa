...
def parse_bids_dataset(
    ...
    **filters
    ...
    try:
        ...
        print(f"Parsed BIDS dataset: {n_subjects} subjects, "
...
def extract_bids_metadata(
    ...
    for file in files:
        ...
        if include_events and 'task' in entities:
            events_path = str(file_path).replace('.nii.gz', '_events.tsv').replace('.nii', '_events.tsv')
            if os.path.exists(events_path):
                try:
                    events_df = pd.read_csv(events_path, sep='\t')
                    record['n_events'] = len(events_df)
                    record['event_types'] = list(events_df.get('trial_type', []))
                except Exception as e:
...
def load_bids_data(
    ...
    **query_filters
    ...
    if not files:
        raise ValueError(f"No files found matching query: {query}. "
    ...
    for i, file_path in enumerate(files):
        try:
            ...
            if data.ndim < 3:
                warnings.warn(f"Skipping {file_path}: expected 3D or 4D data, got {data.ndim}D")
                continue
            elif data.ndim > 4:
                warnings.warn(f"Skipping {file_path}: expected 3D or 4D data, got {data.ndim}D")
                continue
            ...
            if mask is not None:
                ...
                mask_shape = mask_data.shape[:3] if mask_data.ndim >= 3 else mask_data.shape
                
                if data_spatial_shape != mask_shape:
                    warnings.warn(f"Skipping {file_path}: mask shape {mask_shape} "
                ...
                if data.ndim == 4:  # 4D data (x, y, z, t)
                    ...
                    if not np.any(mask_1d):
                        warnings.warn(f"Skipping {file_path}: mask contains no valid voxels")
                        continue
            ...
            if standardize and data_masked.ndim == 2:
                ...
                data_masked = (data_masked - data_masked.mean(axis=1, keepdims=True)) / \
    ...
    if not data_list:
        raise RuntimeError(f"Failed to load any valid data files from {len(files)} candidates. "
    ...
    if len(data_list) == 1:
        ...
        if len(unique_shapes) == 1:
            ...
            raise ValueError(f"Inconsistent data shapes across files. "
    ...
    valid_metadata = metadata[metadata['file_path'].isin(valid_files)].reset_index(drop=True)
...
def load_bids_for_htfa(
    ...
    mask_path: Optional[str] = None,
    **filters: Any
) -> Tuple[np.ndarray, pd.DataFrame, BIDSLayout]:
