...
from .core import TFA, HTFA


def fit(
    data: Union[str, os.PathLike, npt.NDArray],
    ...
    **kwargs
    ...
    if isinstance(data, (str, os.PathLike)):
        ...
        if coords is None:
            raise TypeError("coords parameter is required for multi-subject array input")
        return _fit_arrays(data, coords, n_factors=n_factors, multi_subject=True, **kwargs)
        ...
        return _fit_arrays(data, coords, n_factors=n_factors, multi_subject=use_multi, **kwargs)
    else:
    ...
    if isinstance(data, list):
        ...
        for i, arr in enumerate(data):
            ...
            if arr.shape[0] != n_voxels:
                raise ValueError(f"Subject {i}: expected {n_voxels} voxels, got {arr.shape[0]}")
        
        # Check coordinates match
        if coords.shape[0] != n_voxels:
            raise ValueError(f"Coordinates shape {coords.shape[0]} doesn't match "
        ...
        if multi_subject:
            ...
            model = HTFA(n_factors=n_factors, **kwargs)
            model.fit([data.T])  # Wrap in list for HTFA
        else:
            # Use TFA for single subject
            model = TFA(n_factors=n_factors, **kwargs)
            model.fit(data.T)  # TFA expects (n_timepoints, n_voxels)
...
def _infer_parameters(data: npt.NDArray, **kwargs: Any) -> Dict[str, Any]:
...
def _load_nifti_file(path: Path) -> Tuple[npt.NDArray, npt.NDArray]:
