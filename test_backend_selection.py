...
def test_detection():
    ...
    print(
        f"   - NumPy: {libraries['numpy']['available']} (v{libraries['numpy'].get('version', 'N/A')})"
    )
    print(
        f"   - JAX: {libraries['jax']['available']} (v{libraries['jax'].get('version', 'N/A')})"
    )
    print(
        f"   - PyTorch: {libraries['pytorch']['available']} (v{libraries['pytorch'].get('version', 'N/A')})"
    )
...
def test_selector():
    ...
    print("\n4. Memory estimation:")
    mem_req = selector.estimate_memory_requirement(
        n_voxels=10000, n_times=500, k_components=10
    )
